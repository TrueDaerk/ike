...
func (m *Model) routeOverlayPaste(text string) (cmd tea.Cmd, handled bool) {
	...
	case m.deepLinkChooserOpen(), m.netPairOpen():
		return nil, false // decision prompts, no text input
	...
	case m.runConfigFormOpen():
		return nil, m.pasteRunConfigForm(text)
	case m.breakpointFormOpen():
		return nil, m.pasteBreakpointForm(text)
	...
	case m.scratchCustomExtOpen():
		return nil, m.pasteScratchCustomExt(text)
