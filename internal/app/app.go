...
func buildModel(reg *registry.Registry, cfg host.Config, h *host.Host, mgr *workspace.Manager) Model {
	...
	// Send-outbox drop reporting (#2169): a tripped bound leaves a line in
	// debug.log; re-wiring on a project switch keeps the live host.
	h.SetDiagLog(logDiagnostic)
...
// StartWatcherAsync is StartWatcher with the recursive watch registration off
// the caller (#2260): the walk over the whole project tree is the biggest
// single pre-first-frame cost on a large repository, so main.go must not pay
// it before the first paint. Missing the first seconds of events is covered
// by the explorer's poll and the initial VCS snapshot; a Start superseded by
// a project switch abandons its walk (watch.Service.owns). main.go-only — the
// switch path and tests keep the synchronous StartWatcher, so a switch's
...
func (m *Model) installEmitter(key string) {
	if inst := m.activeWS().Panes.Get(key); inst != nil && inst.Kind() == pane.KindEditor {
		...
		for _, ed := range inst.Editors() {
			ed.SetEmitter(editorEmitter{host: m.host, watcher: m.watcher, nav: m.navHist, edits: m.editRing, root: m.activeWS().Root, key: key, previews: m.previewBound})
...
func (m Model) updateMsg(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case ShowNotificationHistoryMsg:
		// notifications.history (palette / cmd+alt+n / status counter click):
		// the notification center over the history ring (#2152). Opening marks
		// everything seen — the status line counter resets (#101).
		m.openNotifCenter()
		return m, nil
	...
	case diff.EditRequestMsg:
		...
		ed.SetEmitter(editorEmitter{host: m.host, watcher: m.watcher, nav: m.navHist, edits: m.editRing, root: m.activeWS().Root, key: msg.Key, previews: m.previewBound})
...
func (m *Model) activateTab(inst *pane.Instance, idx int) {
	...
	if ed := inst.Editor(); ed != nil && ed.HasFile() {
		...
		m.fileFrec.Record(frecency.Key(ed.Path()))
	}
}
