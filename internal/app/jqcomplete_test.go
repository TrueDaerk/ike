...
func dismissJQPopup(m Model) Model {
	if m.jqPlay != nil && m.jqPlay.comp != nil {
		m = drainKey(m, tea.KeyPressMsg{Code: tea.KeyEscape})
	}
	return m
}

// jqCompLabels flattens the open popup's labels for assertions.
func jqCompLabels(m Model) []string {
	if m.jqPlay == nil || m.jqPlay.comp == nil {
		return nil
	}
	out := make([]string, len(m.jqPlay.comp.items))
	for i, it := range m.jqPlay.comp.items {
		out[i] = it.Label
	}
	return out
}
...
func TestJQCompletionOpensOnDotAndFilters(t *testing.T) {
	m := openJQ(t, jqApp(t, `{"name":"ike","nation":"x","other":1}`))
	m.jqPlay.program, m.jqPlay.pos = "", 0
	m = typeInto(m, ".")
	if got := jqCompLabels(m); strings.Join(got, " ") != "name nation other" {
		t.Fatalf("`.` offered %v, want the top-level keys", got)
	}
	m = typeInto(m, "na")
	if got := jqCompLabels(m); strings.Join(got, " ") != "name nation" {
...
func TestJQCompletionAccept(t *testing.T) {
	m := openJQ(t, jqApp(t, `{"name":"ike","nation":"x"}`))
	m.jqPlay.program, m.jqPlay.pos = "", 0
	m = typeInto(m, ".nam")
	if m.jqPlay.comp == nil {
	...
	if got := m.jqPlay.program; got != ".name" {
...
func TestJQCompletionDismiss(t *testing.T) {
	...
	if got := m.jqPlay.program; got != ".na" {
...
func TestJQCompletionNestedThroughArrays(t *testing.T) {
	m := openJQ(t, jqApp(t, `{"items":[{"id":7},{"id":8,"name":"x"}]}`))
	m.jqPlay.program, m.jqPlay.pos = "", 0
	m = typeInto(m, ".items[].")
	if got := jqCompLabels(m); strings.Join(got, " ") != "id name" {
...
func TestJQCompletionQuotedKeyAccept(t *testing.T) {
	m := openJQ(t, jqApp(t, `{"a b":42}`))
	m.jqPlay.program, m.jqPlay.pos = "", 0
	m = typeInto(m, ".")
	m = drainKey(m, tea.KeyPressMsg{Code: tea.KeyEnter})
	if got := m.jqPlay.program; got != `."a b"` {
		t.Fatalf("accept wrote %q, want the quoted access", got)
	}
	if got := m.jqPlay.result.Text(); got != "42" {
...
func TestJQCompletionBuiltins(t *testing.T) {
	...
	m = drainKey(m, tea.KeyPressMsg{Code: tea.KeyEnter})
	if got := m.jqPlay.program; got != "select" {
...
func TestJQCompletionManualRequest(t *testing.T) {
	...
	if got := jqCompLabels(m); len(got) == 0 {
