...
func BasePages(themes, lightThemes, darkThemes []string, extraThemes ...theme.Theme) []Page {
	...
	return []Page{
		...
		{Title: "Scratch Files", Description: "The Scratch Files tool window (#1932): a slim list of the scratch store below the editor, newest first, opening and deleting scratches without leaving the IDE.", Entries: []Entry{
			{Key: "scratch.panel", Type: Bool, Title: "Show scratch panel", Description: "Open the Scratch Files panel below the editor on start; off (the default) costs no editor rows and leaves the panel to the \"Scratch Files\" command, which shows and hides it for the session either way", Scope: config.UserScope},
			{Key: "scratch.panel_height", Type: Int, Title: "Scratch panel height", Description: "Height in terminal rows the Scratch Files panel opens at, its border and title row included (8 shows four scratches plus the key hints). Dragging the divider above the panel resizes it afterwards, and that height persists with the window layout", Scope: config.UserScope, Min: 5, Max: 60},
		}},
		{Title: "Tool Layout", Description: "Named slot template pinning tool windows to exact layout positions (#1897). The template is an ASCII grid: each row is one entry, every cell names a slot by a single letter, E is the editor region. Assigned tools always open at their slot; unassigned ones keep their home position or the adaptive split.", Entries: []Entry{
			...
			{Key: "tools.layout.assign", Type: List, Title: "Slot assignments", Description: "SLOT=tool entries pinning tools to template slots (\"X=explorer, T=lazygit\"): the tool is a custom tool's name or a built-in id (explorer, vcs, debug, problems, structure, usages, http, breakpoints, run, tests, issues, dom, scratch, terminal). \"terminal\" pins fresh terminal panes (not the popup overlay); run and debug place independently. Several tools may share a slot — the first open materializes the pane, later ones join it as tabs; each tool takes at most one slot. While typing, the valid slot letters and tool ids are listed under the input", Scope: config.UserScope, EntryHints: assignHints, ValidateEntry: assignValidate},
		}},
