...
func BasePages(themes, lightThemes, darkThemes []string) []Page {
	return []Page{
		{Section: "CORE", Title: "Editor", Description: "How text is edited: indentation, saving, wrapping and the gutter. Language-specific overrides come from .editorconfig when it is enabled.", Entries: []Entry{
			...
			{Key: "editor.markdown_rendering", Type: Bool, Title: "Markdown rendering", Description: "Render Markdown inline styles, conceal markers (revealed only while the caret is inside them) and draw pipe tables with box characters while the cursor is outside the table", Scope: config.UserScope},
