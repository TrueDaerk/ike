...
func BasePages(themes, lightThemes, darkThemes []string, extraThemes ...theme.Theme) []Page {
	...
	return []Page{
		...
		{Title: "Conceal & Hints", Description: "The conceal and inline-hint families: what the editor draws in place of — or after — the raw bytes. These are the config defaults; a per-view toggle from the editor (Toggle Timestamp Decoding, …) overrides them for that buffer only. The file rules below gate every family by path.", Entries: []Entry{
			...
			{Key: "editor.vault", Type: Bool, Title: "Vault stand-in", Description: "Collapse an inline Ansible !vault value in a YAML buffer onto one row, ⟨vault AES256 · 6 lines⟩ (with the vault id when the header names one); the raw header and hex lines reappear with the caret inside the block. g? on the block decrypts it into the explain popover when a password source is configured (Ansible Vault page), and alt+enter offers Edit vault value, Encrypt value with Ansible Vault and Decrypt vault value to plain text. Toggle per view via Toggle Vault Stand-In; the file rules below know the family as vault", Scope: config.UserScope},
		...
		{Title: "Explorer", Description: "The project file tree: what it shows, how it is sorted and what it hides. Excluding an entry here hides it from the tree only; go-to-file and find-in-path still reach it.", Entries: []Entry{
			{Key: "explorer.show_hidden", Type: Bool, Title: "Show hidden files", Description: "List dot files and dot directories in the tree; entries matched by the exclude patterns below stay hidden either way. The explorer's . chord writes this key, so the toggle applies to every project", Scope: config.UserScope},
		...
		{Title: "PHP", Description: "PHP trait-consumer intelligence: IKE's own workspace-wide declaration index knows which classes use which traits, so inside a trait body $this-> resolves against the consumers' members where the language server is blind. Everything here is additive to Intelephense; with the index off, behaviour equals the server's alone.", Entries: []Entry{
			{Key: "php.trait_index", Type: Bool, Title: "Trait consumer index", Description: "Build the PHP declaration index and use it inside trait bodies (completion, diagnostics, navigation, references, rename). Off drops the index at once; on rebuilds it", Scope: config.UserScope},
			{Key: "php.index.parent_depth", Type: Int, Title: "Parent depth", Description: "How many parent classes of a trait's consumers contribute members to the trait's scope: 1 is the direct parent, 0 none. Framework base classes beyond the depth stay with the language server", Scope: config.UserScope, Min: 0, Max: config.PHPParentDepthMax},
			{Key: "php.index.include_vendor", Type: Bool, Title: "Index vendor/", Description: "Read vendor/ too. Off by default: Intelephense already covers installed packages, and a large vendor tree costs scan time", Scope: config.UserScope},
			{Key: "php.index.max_files", Type: Int, Title: "Maximum files", Description: "How many PHP files the project walk reads before it stops; the status report flags a truncated scan. At least 100", Scope: config.UserScope, Min: config.PHPMaxFilesMin, Max: config.PHPMaxFilesMax},
		}},
		{Title: "Language Support", Description: "The LSP subsystem's global switches: whether language servers run at all, what they may install, which requests fire on their own, and when a server that stays silent after a project switch is reported. Per-language server commands live on the Language Servers page.", Entries: []Entry{
			...
			{Key: "lsp.completion_delay_ms", Type: Int, Title: "Completion delay", Description: "Milliseconds an identifier character waits for the next keystroke before the completion request goes out, so a typing burst asks the server and the local indexes once, at the resting position, instead of once per character. 0 requests immediately; server trigger characters (\".\") and the manual ctrl+space request never wait", Scope: config.UserScope, Min: 0, Max: 2000},
			{Key: "completion.case_sensitivity", Type: Enum, Title: "Completion case sensitivity", Description: "Case rule the completion popup's hump filter holds the typed prefix to. Candidates match JetBrains-style: every typed character either continues the previous match or starts a word segment (camelCase hump, after _ - ., a letter/digit change), so \"dao\" offers DataAccessObject and \"my\" no longer offers empty or summary. \"first_letter\" (the IntelliJ default) lets a lowercase character match either case while an uppercase one only matches an uppercase character, so \"DataA\" cannot match database; \"none\" ignores case entirely; \"all\" matches every character exactly", Scope: config.UserScope, Options: []string{"none", "first_letter", "all"}},
			...
			{Key: "lsp.warmup_notice_ms", Type: Int, Title: "Silent server notice", Description: "Milliseconds a project switch waits for the first diagnostics of the newly opened documents before it reports that the language server has not answered. A switch that opens files of a server language waits for that first publish; a missing, dead or misconfigured server never sends one, and without this notice the editor just stays diagnostic-blind without saying so. The notification names the language and offers Restart Language Servers and the LSP Doctor as follow-ups, and it is recorded in the notification history like any other. Warm-ups normally land within a few seconds, so the default leaves room for a cold index. 0 turns the notice off, and so does switching language servers off entirely", Scope: config.UserScope, Min: 0, Max: 600000},
		...
		{Title: "Appearance", Description: "How IKE looks: the color scheme, the menu bar, the horizontal scroll marks, the pane-number badges and the size of centered popups.", Entries: []Entry{
			...
			{Key: "layout.pane_slots", Type: List, Title: "Reserved pane numbers", Description: "\"tool=number\" entries pinning a pane number to a tool window (\"terminal=2, vcs=3\"), so ctrl+N reaches that tool however many editors are open — and opens it when it is closed. The tool is a built-in window (terminal, vcs, problems, structure, usages, …) or a custom tool's name (\"lazygit=6\"), where the chord opens that tool at its configured position; a custom tool named like a built-in window loses the id to the window. The explorer is always 1 and takes no entry; numbers run from 2 to 9, each used once, each tool named once. The document panes (editor, diff, markdown, image, …) take the numbers after the highest reserved one, so a short table leaves them more chords. While typing, the free tools (custom ones marked) and numbers are listed under the input", Scope: config.UserScope, EntryHints: paneSlotHints, ValidateEntry: paneSlotValidate},
		...
		{Title: "Command Palette", Description: "The command palette overlay: how many rows it shows, which mode a query without a prefix rune lands in, and what happens to commands belonging to another pane.", Entries: []Entry{
			...
			{Key: "palette.recent.ranking", Type: Enum, Title: "Recent files ranking", Description: "How the recent-files dialog (cmd+e) orders its files and projects: \"recency\" (the default) lists them newest first, \"frecency\" blends in how often each was opened", Scope: config.UserScope, Options: []string{"recency", "frecency"}},
			{Key: "palette.hint_keybind", Type: Bool, Title: "Keybind hint after a pick", Description: "After running a command from the palette, toast the chord it is bound to in the focused pane (\"also: cmd+shift+a\"); once the same unbound command was picked three times in a session, offer Bind a Key for the Last Palette Command instead", Scope: config.UserScope},
		}},
		...
		{Title: "Files & Session", Description: "Opening, watching and restoring files: which project comes back on start, how external changes are handled and when a file counts as too large for language features.", Entries: []Entry{
			...
			{Key: "files.large_file_kb", Type: Int, Title: "Large file threshold (KB)", Description: "Above this size, highlighting and language features are disabled for the file; 0 disables the size guard. Applies to subsequently opened or reloaded files", Scope: config.UserScope, Min: 0},
			...
			{Key: "files.large_file_lines", Type: Int, Title: "Large file threshold (lines)", Description: "Above this line count, highlighting and language features are disabled for the file; 0 disables the line guard. Applies to subsequently opened or reloaded files", Scope: config.UserScope, Min: 0},
		...
		{Title: "Usage Telemetry", Description: "Local-only usage recording: structural events — which commands ran and how they were invoked, which chords resolved (or didn't), how panes and tabs are arranged — appended as JSONL under ~/.ike/telemetry for offline analysis with jq or scripts. Nothing ever leaves the machine and no event carries content: no typed text, no file contents, no clear-text paths.", Entries: []Entry{
			...
			{Key: "statusline.project_time", Type: Enum, Title: "Project time in the status line", Description: "Show today's active time in the current project as a status-line segment, aggregated from the same local usage log the Time tool window reports on (time.toggle). Needs telemetry.enabled to have anything to read; the segment refreshes about once a minute and clicking it opens the Time window", Scope: config.UserScope, Options: []string{"off", "on"}},
		}},
		{Title: "Timeline", Description: "The per-file Timeline: one chronological list of a file's local-history snapshots and the git commits that touched it, opened with Show Timeline.", Entries: []Entry{
		...
		{Title: "Terminal", Description: "The integrated terminal and what it offers while you type at the shell prompt.", Entries: []Entry{
			...
			{Key: "terminal.scrollback_lines", Type: Int, Title: "Scrollback lines", Description: "Lines of scrollback each terminal session keeps; the main memory cost per terminal pane. Applies to new sessions and, on lowering, trims live ones forward — already-trimmed history is not restored by raising it", Scope: config.UserScope, Min: 100, Max: 1000000},
		...
		{Title: "Ansible Vault", Description: "Transparent Ansible Vault editing: a $ANSIBLE_VAULT; file opens decrypted into an editable buffer and is re-encrypted on save, so the plaintext never touches the disk. Needs a password source: the ANSIBLE_VAULT_PASSWORD or ANSIBLE_VAULT_PASSWORD_FILE environment variable, or the password file below.", Entries: []Entry{
		...
		{Title: "Playgrounds", Description: "The buffer playgrounds: live jq, yq and xmq query lines over the focused document. jq and yq run in-process; the xmq playground shells out to the xmq CLI (github.com/libxmq/xmq).", Entries: []Entry{
			{Key: "playground.xmq.path", Type: Path, Title: "xmq binary", Description: "The xmq executable the XML/HTML playground runs; \"~\" expands, and a bare name resolves on PATH. Empty resolves \"xmq\" on PATH. With no resolvable binary the playground answers its open with an install-hint dialog (brew install xmq) instead of opening", Scope: config.UserScope},
		}},
		...
		{Title: "Network Links", Description: "The network counterpart of ike:// deep links: a TCP endpoint other devices pair with once (a six-digit PIN shown in a popup) and then drive over a line protocol — switch project, open a file at a line, show a tool window. Off until enabled; a change restarts the listener.", Entries: []Entry{
		...
		{Title: "Markdown Preview", Description: "The rendered markdown and HTML preview panes: how the diagram fences inside a document — ```mermaid blocks in READMEs and design docs — are drawn, which view an opened HTML file starts in, whether an HTML page's local images show inline, how much of a large page the HTML preview renders, and which headless browser its screenshot mode uses.", Entries: []Entry{
			...
			{Key: "preview.html_open_mode", Type: Enum, Title: "Open HTML files in", Description: "The view an opened .html/.htm/.xhtml file (or .html.gz) starts in: \"preview\" shows the rendered page in the file's tab, \"source\" the editor. The [Source] [Preview] buttons in the tab's bottom-left corner and \"HTML: toggle Source/Preview\" switch either way. A jump to a line — go to definition, a search result, a problem, file:line — always opens in source, so the caret lands where it points", Scope: config.UserScope, Options: []string{config.HTMLOpenPreview, config.HTMLOpenSource}},
		...
		{Title: "Tests", Description: "The Test Results tool window: a structured tree of a test run's outcomes for languages with an output parser (Go, Python/pytest).", Entries: []Entry{
		...
		{Title: "Forge", Description: "The code forge behind the Issues tool window (GitHub through gh, Gitea/Forgejo through tea): how often IKE re-reads it in the background, whether it does so at all while nobody is looking, and whether the status line names the issue behind the current branch.", Entries: []Entry{
			...
			{Key: "statusline.branch_issue", Type: Enum, Title: "Branch issue in the status line", Description: "Show the issue behind the current branch as a status-line segment — \"#2544 status line: show the issue behind…\" — resolved from the same forge listing the Issues tool window and its persistent cache hold, so it costs no extra request and refreshes with the background poll. A branch whose issue is not in the listing shows the bare number; clicking the segment (or \"Open Current Branch Issue\") opens that issue's detail in the Issues window", Scope: config.UserScope, Options: []string{"off", "on"}},
			{Key: "statusline.branch_issue_pattern", Type: String, Title: "Branch issue pattern", Description: "Regular expression matched against the current branch name for the segment above; its first capture group is read as the issue number. The default ^issue/(\\d+) is IKE's own change-workflow naming; a repository branching \"feature/ISSUE-12-title\" would use ^feature/ISSUE-(\\d+). A pattern that does not compile, or that captures nothing, is rejected", Scope: config.UserScope, ValidateString: branchIssuePatternValidate},
		}},
		{Title: "Diff Viewer", Description: "The diff pane: where a diff-open lands in the layout, how much unchanged context it keeps around a change, and whether it compares whitespace at all. The last two are starting points — c/o expand context and w flips whitespace handling on the open diff; the toggle writes its new state back here.", Entries: []Entry{
			...
			{Key: "diff.placement", Type: Enum, Title: "Diff placement", Description: "Where a diff-open (diff.files, the VCS panel's HEAD and commit diffs, local history, compare-with-clipboard, the HTTP response diff) puts its viewer. \"focused\" opens it as a tab of the pane you are working in — the focused editor pane, or the most recently focused one when a tool window has the keyboard — so the layout keeps its shape and the diff appears where the eye already is; the pane's file tabs stay, an empty scratch pane is taken over in place, and a second diff from that pane reuses its diff tab. \"split\" restores the older behaviour: every diff carves off a new pane to the right of the active editor", Scope: config.UserScope, Options: []string{"focused", "split"}},
		...
		{Title: "HTTP Client", Description: "The .http client's re-run and compare behaviour: what a response diff leaves out, and whether re-running a request opens the comparison with the run before it by itself — plus the response viewer's syntax-highlight cap, how long a dispatch may run before it times out, and when a finished dispatch reports itself as a notification.", Entries: []Entry{
			...
			{Key: "http.slow_threshold_ms", Type: Int, Title: "Slow flight highlight", Description: "Milliseconds past which a finished dispatch counts as slow in the response pane: its header then shows the duration and the timing phase that took the longest (dns, connect, tls, ttfb or transfer) in the warning slot, and the off-screen completion notice names that phase too. 0 turns the highlight off. Independent of the slow response notice above, which decides whether a notice is sent at all", Scope: config.UserScope, Min: 0, Max: 600000},
			{Key: "http.timeout_ms", Type: Int, Title: "Request timeout", Description: "Milliseconds one dispatch may run before it is given up on: past this the request ends with an explicit \"timed out after N s\" in the response pane instead of hanging until you cancel it, and the pane header counts the wait down against this limit while the request is out. It is the default only — a single request overrides it with a \"# @timeout 5s\" directive in its .http block, and a .curlrc \"max-time\" wins over the setting too. A recognised stream (SSE/NDJSON) sheds the deadline once its headers are in and ends on idleness instead, so a long-lived connection is not cut off", Scope: config.UserScope, Min: 1000, Max: 600000},
		}},
		{Title: "Scratch Files", Description: "The explorer's Scratches section: the scratch store listed below the file tree behind a divider, opened, renamed and deleted with the explorer's own keys and dialogs.", Entries: []Entry{
		...
		{Title: "Issues Window", Description: "The forge Issues tool window: two full-area tabbed views — the issue list and the pull-request list — with a fuzzy filter, a label picker, an open/closed/all state filter, sort orders and a grouping toggle. These settings decide only what a freshly opened pane starts with; switching the tab, the order or a filter by hand wins for the rest of the session.", Entries: []Entry{
		...
		{Title: "Performance HUD", Description: "The built-in performance HUD: a floating overlay with message rates, per-pane render cost, goroutines and memory, toggled with the Performance HUD command. It only measures while it is shown.", Entries: []Entry{
		...
		{Title: "Forge Notifications", Description: "How prominently each kind of forge event announces itself: dialog is a centered, dismissable dialog over the workspace, badge only marks the status line unread, toast is the ordinary bottom-right notice, off records the event in the notification history alone. While you are typing, a dialog is held back and shows as the badge instead.", Entries: []Entry{
		...
		{Title: "Dependencies", Description: "The Dependencies tool window's project scan: declared dependencies per manifest with latest versions and vulnerabilities from the local toolchain.", Entries: []Entry{
			{Key: "deps.auto_scan", Type: Bool, Title: "Scan dependencies on project open", Description: "Run one background toolchain scan (outdated versions and vulnerability audit) when a project with dependency manifests opens; off leaves scans to the deps.refresh and deps.audit commands", Scope: config.UserScope},
		}},
		{Title: "Marketplace Catalog", Description: "Where the plugin marketplace fetches its catalog from. The Marketplace page installs from whatever this URL serves.", Entries: []Entry{
			...
			{Key: "marketplace.auto_check", Type: Bool, Title: "Check for plugin updates", Description: "Compare installed plugins against the catalog on IDE start and announce how many updates are available. The check runs in the background at most once a day and stays silent when the catalog cannot be reached; opening the Marketplace page always checks regardless", Scope: config.UserScope},
		}},
	}
}
...
// branchIssuePatternValidate is the form check for
// statusline.branch_issue_pattern (#2544): the same rule the config validator
// applies, but refusing the typed value instead of quietly falling back to
// the default — a mistyped regexp would otherwise leave the segment blank
// with no explanation anywhere.
func branchIssuePatternValidate(v string) string { return config.ValidateBranchIssuePattern(v) }
