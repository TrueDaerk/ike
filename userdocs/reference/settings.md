...
| HTML preview render budget (KB) | `preview.html_render_budget_kb` | integer (64–65536) | `2048` | user | How much of an HTML page the preview renders, in KiB of source. Rendering runs off the UI loop and a newer edit cancels a render still in flight; past the budget it stops and the preview ends with a "… truncated after N KB" line, so a multi-MB report neither freezes IKE nor keeps the pane busy. The editor still holds the whole file |
| HTML preview browser | `preview.html_browser` | path | *(empty)* | user | Headless browser the HTML preview's browser mode (b in the pane, "HTML preview: render in browser") screenshots the page with: a Chrome, Chromium or Edge executable; "~" expands and a bare name resolves on PATH. Empty auto-detects google-chrome, chromium, chrome or microsoft-edge on PATH (and the standard app bundles on macOS). A directory is refused — for an .app bundle name the binary inside Contents/MacOS. Without a browser the toggle stays in text mode with a notice |
| HTML preview browser timeout (s) | `preview.html_browser_timeout_s` | integer (1–300) | `20` | user | Seconds one browser-mode screenshot may take before the browser is killed and the preview falls back to text mode with a notice |
...
| Completion delay | `lsp.completion_delay_ms` | integer (0–2000) | `100` | user | Milliseconds an identifier character waits for the next keystroke before the completion request goes out, so a typing burst asks the server and the local indexes once, at the resting position, instead of once per character. 0 requests immediately; server trigger characters (".") and the manual ctrl+space request never wait |
| Completion case sensitivity | `completion.case_sensitivity` | enum: `none`, `first_letter`, `all` | `first_letter` | user | Case rule the completion popup's hump filter holds the typed prefix to. Candidates match JetBrains-style: every typed character either continues the previous match or starts a word segment (camelCase hump, after _ - ., a letter/digit change), so "dao" offers DataAccessObject and "my" no longer offers empty or summary. "first_letter" (the IntelliJ default) lets a lowercase character match either case while an uppercase one only matches an uppercase character, so "DataA" cannot match database; "none" ignores case entirely; "all" matches every character exactly |
...
| Slow flight highlight | `http.slow_threshold_ms` | integer (0–600000) | `2000` | user | Milliseconds past which a finished dispatch counts as slow in the response pane: its header then shows the duration and the timing phase that took the longest (dns, connect, tls, ttfb or transfer) in the warning slot, and the off-screen completion notice names that phase too. 0 turns the highlight off. Independent of the slow response notice above, which decides whether a notice is sent at all |
| Request timeout | `http.timeout_ms` | integer (1000–600000) | `30000` | user | Milliseconds one dispatch may run before it is given up on: past this the request ends with an explicit "timed out after N s" in the response pane instead of hanging until you cancel it, and the pane header counts the wait down against this limit while the request is out. It is the default only — a single request overrides it with a "# @timeout 5s" directive in its .http block, and a .curlrc "max-time" wins over the setting too. A recognised stream (SSE/NDJSON) sheds the deadline once its headers are in and ends on idleness instead, so a long-lived connection is not cut off |
...
| Announced name | `network.name` | string | *(empty)* | user | The instance label other devices see when browsing ("geants-mac"); empty announces the host's short name. One DNS label: no dots, no control characters, at most 63 bytes |
...
| Branch issue in the status line | `statusline.branch_issue` | enum: `off`, `on` | `off` | user | Show the issue behind the current branch as a status-line segment — "#2544 status line: show the issue behind…" — resolved from the same forge listing the Issues tool window and its persistent cache hold, so it costs no extra request and refreshes with the background poll. A branch whose issue is not in the listing shows the bare number; clicking the segment (or "Open Current Branch Issue") opens that issue's detail in the Issues window |
| Branch issue pattern | `statusline.branch_issue_pattern` | string | `^issue/(\d+)` | user | Regular expression matched against the current branch name for the segment above; its first capture group is read as the issue number. The default ^issue/(\d+) is IKE's own change-workflow naming; a repository branching "feature/ISSUE-12-title" would use ^feature/ISSUE-(\d+). A pattern that does not compile, or that captures nothing, is rejected |
