...
type: concept
title: Data Viewer
description: "#1764/#1765/#1766 — table files (SQLite .db/.sqlite/.sqlite3, DuckDB .duckdb/.ddb and Parquet .parquet/.pqt, by extension or magic) open as a table sidebar plus a paged read-only grid instead of a binary text buffer; the pane speaks a small backend interface, SQLite and Parquet ride pure-Go readers and DuckDB the duckdb CLI so the build stays cgo-free."
resource: internal/dataview
tags: [architecture, database, sqlite, duckdb, parquet, viewer, pane, read-only, grid]
timestamp: 2026-08-10T20:40:00Z
---

# Data Viewer (#1764, #1765, #1766)
...
- `internal/datasrc` — the backend contract plus the engines. The `Source`
  ...
  **no SQL of its own** — that is the seam SQLite, DuckDB and Parquet share,
  and it is why the Parquet backend, which has no query engine behind it at
  all, needed no pane change.
