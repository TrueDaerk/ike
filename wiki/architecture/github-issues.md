...
- **`f`** (with **`/`** and the shared find chord `cmd+f` / `ctrl+f` (#2409)
  ...
  assignees and author (head branch on the PR tab). `cmd+g` / `cmd+shift+g`
  then walk the narrowed list behind the open overlay while the input keeps
  the keyboard (#2410), skipping the group headers; the match row shows the
  counter with its `(wrapped)` marker, and every pattern edit drops it.
...
- Since #2156 the syntax itself is not the Issues pane's own: the tokenizer,
  the quoting rules, the schema validation and the error wording live in
  `internal/filterexpr`, shared with the Problems and Usages panes and the
  TODO index — see [List Filter Syntax](./list-filters.md).
  `internal/issuefilter` is the Issues *dialect*: the `Schema` naming `is:`,
  `label:` and `sort:` plus the `Spec` shape the config validator and the
  settings form read. The live match input resolves its qualifiers against
  that same schema, so the two readers cannot drift on which values exist.
...
- **Background refresh** (#2085) — the app's poll service pushes a fresh
  ...
  the content stays current without pressing `r`. While this window is *not*
  open the poll runs at a fifth of that rate (#2488) — the status-line unread
  badge still moves, nothing else depends on it — and opening the window
  restores the configured interval, refetching at once when the stretched
  cadence let the listing go stale. A poll result is applied so
