...
#2535. The document survives a project switch parked in its workspace (#777),
so the mode does too. `performSwitch` discards the whole model and builds a
fresh one, which used to drop the playground with it; now the open state
parks in the workspace's `wsExtras` beside the debug session and the popup
terminal (`parkPlayground`, `internal/app/playground_switch.go`), and
`buildModel` mounts it again when that workspace resumes
(`resumePlayground`): query, caret, result, history position, multi-line
state and the result buffer's scroll come back as they were, and the pane
renders it at once since it still shows the queried document. The other
project meanwhile shows no playground at all — the fresh model's slot is
empty, so nothing of project A's query line leaks into project B. The
response-pane playground behaves the same as long as the response pane
survives the switch; it is matched by instance, exactly as for a tab switch.

Three things do not park as-is. **Work in flight** is abandoned at the park —
an evaluation or an off-loop parse started before the switch would deliver
its message to the other project's model, where it is dropped as belonging
to no open playground — and re-driven on resume (`resumePlayRun`, after the
size pass): a pending run re-runs the current program, an interrupted parse
re-reads the response or the followed file. A **followed file whose bytes
changed while parked** is re-read the way an external change is (#2356): the
resume runs after the buffer reconcile (#1515), and no watcher event ever
reached the playground in that window. The **chrome** — the result buffer's
palette and editor config — is re-threaded from the incoming project's
settings layer, like every resumed editor's. The program history the state
points at is session state (#1977) and rides model-to-model with the switch,
so the pointer stays the one live list.

The "closes when its document leaves" rule is untouched: a resumed
playground is the same state under the same `syncPlaygroundSource` hook, so
closing its tab, its pane or its file still ends it.
