...
* [Data Viewer](/architecture/data-viewer.md) - database files (SQLite by extension or magic) as a table sidebar plus a paged read-only grid; backend behind a small Source interface so DuckDB/Parquet plug in, file opened mode=ro so writers are never blocked (#1764)
