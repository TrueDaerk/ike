...
| SQL | sqls (`go install github.com/sqls-server/sqls@latest`) | Maintained Go binary, LSP over stdio by default, no Node dependency; keyword/function completion and formatting without a DB, richer completion with a `.sqls/config.yml` connection. Replaced sql-language-server, which crashes on startup under Node ≥ 26 (#1066). **Reformat** uses IKE's built-in SQL formatter by default (#1403, [Formatter Registry](./format.md)) — sqls formatting returns via `[format.sql] builtin = false`. |
...
| XML | — (none) | The mainstream server (lemminx) ships as a JVM application, so no server (#1253) — but no longer highlighting-only: **Reformat** works via the built-in XML pretty-printer (#1404, [Formatter Registry](./format.md)), covering `.xml` and the dialects `.xsd`/`.xsl`/`.xslt`/`.svg`/`.plist`/`.wsdl`/`.csproj` & co. |
...
| Markdown | marksman (`marksman server`, `brew install marksman`) | Single static binary; completes link targets, heading anchors, wiki-links, reference labels. Prose keeps the word-index source. Root markers are `.marksman.toml`, then `.git`; marksman resolves root-relative links (`/doc.md`) against that workspace root and skips `.gitignore`d files when the root is a git repo, so a nested doc tree wants its own `.marksman.toml` to avoid false "non-existent document" diagnostics — IKE ships one in `wiki/` (#1726). |
