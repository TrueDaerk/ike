...
// TestINIEpochValues (#1684): a Unix epoch on the value side of a `key = value`
// pair decodes.
func TestINIEpochValues(t *testing.T) {
	var stamps []lang.Span
	for _, s := range iniSpans([]string{"[cache]", "purged = 1722945600"}) {
		if s.Replace != "" {
